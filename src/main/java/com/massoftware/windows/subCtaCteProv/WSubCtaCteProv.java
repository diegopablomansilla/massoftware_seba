...
public class WSubCtaCteProv extends Window {

	private static final long serialVersionUID = -6410625501465383928L;

	// -------------------------------------------------------------

	private BeanItem<SubCtaCteProvFiltro> filterBI;
	private BeanItemContainer<SubCtaCteProv> itemsBIC;
	...
	public WSubCtaCteProv() {
		...
		try {
			...
			numeroTXTHL = UtilUI.buildTXTHLInteger(filterBI, "subCtaCteProv",
					"Sub Cta Cte Prov", false, 5, -1, 3, false, false, null, false,
					UtilUI.EQUALS, 0, 255);
			...
			itemsGRD.setColumns(new Object[] { "subCtaCteProv", "nombre" });

			UtilUI.confColumn(itemsGRD.getColumn("subCtaCteProv"), "Nro.", true, 80);
			UtilUI.confColumn(itemsGRD.getColumn("nombre"), "Nombre", true, 250);
			...
			order.add(new SortOrder("subCtaCteProv", SortDirection.ASCENDING));
			...
			content.addComponents(itemsGRD, filaBotoneraPagedHL,
...
//			content.setComponentAlignment(filaFiltroHL, Alignment.MIDDLE_CENTER);
	...
	private void buildContainersItems() throws Exception {
		...
		filterBI = new BeanItem<SubCtaCteProvFiltro>(new SubCtaCteProvFiltro());
		itemsBIC = new BeanItemContainer<SubCtaCteProv>(SubCtaCteProv.class,
				new ArrayList<SubCtaCteProv>());
	}
										...
										try {
											if (yes) {
												if (itemsGRD.getSelectedRow() != null) {
													...
													SubCtaCteProv item = (SubCtaCteProv) itemsGRD
															.getSelectedRow();
	...
	private void modificarBTNClick() {
		try {
			...
			if (itemsGRD.getSelectedRow() != null) {
				...
				SubCtaCteProv item = (SubCtaCteProv) itemsGRD.getSelectedRow();
				item.getSubCtaCteProv();
	...
	private void loadData() {
		try {
			...
			List<SubCtaCteProv> items = queryData();

			itemsBIC.removeAllItems();

			for (SubCtaCteProv item : items) {
	...
	private List<SubCtaCteProv> queryData() {
		try {
			...
			List<SubCtaCteProv> items = mockData(limit, offset,
		...
		return new ArrayList<SubCtaCteProv>();
	}

	// metodo que realiza el delete en la base de datos
	private void deleteItem(SubCtaCteProv item) {
		try {

			for (int i = 0; i < itemsMock.size(); i++) {
				if (itemsMock.get(i).getSubCtaCteProv().equals(item.getSubCtaCteProv())) {
					itemsMock.remove(i);
					return;
				}
			}
	...
	List<SubCtaCteProv> itemsMock = new ArrayList<SubCtaCteProv>();

	private List<SubCtaCteProv> mockData(int limit, int offset, SubCtaCteProvFiltro filtro) {

		if (itemsMock.size() == 0) {

			for (int i = 0; i < 500; i++) {

				SubCtaCteProv item = new SubCtaCteProv();

				item.setSubCtaCteProv(i);
		...
		ArrayList<SubCtaCteProv> arrayList = new ArrayList<SubCtaCteProv>();

		for (SubCtaCteProv item : itemsMock) {

			boolean passesFilterNumero = (filtro.getSubCtaCteProv() == null || item
					.getSubCtaCteProv().equals(filtro.getSubCtaCteProv()));
