...
public class WCodigosConvenioMultilateral extends Window {

	private static final long serialVersionUID = -6410625501465383928L;

	// -------------------------------------------------------------

	private BeanItem<CodigosConvenioMultilateralFiltro> filterBI;
	private BeanItemContainer<CodigosConvenioMultilateral> itemsBIC;
	...
	public WCodigosConvenioMultilateral() {
		super();
		...
		try {
			...
			numeroTXTHL = UtilUI.buildTXTHLInteger(filterBI, "codigoCm",
			...
			itemsGRD.setColumns(new Object[] { "codigoCm", "codigoConvenio", "descripcion" });

			UtilUI.confColumn(itemsGRD.getColumn("codigoCm"), "Nro.", true, 50);
			UtilUI.confColumn(itemsGRD.getColumn("codigoConvenio"), "Cod. Convenio", true, 200);
			UtilUI.confColumn(itemsGRD.getColumn("descripcion"), "Descripción", true, 80);
			...
			order.add(new SortOrder("codigoCm", SortDirection.ASCENDING));
	...
	private void buildContainersItems() throws Exception {
		...
		filterBI = new BeanItem<CodigosConvenioMultilateralFiltro>(new CodigosConvenioMultilateralFiltro());
		itemsBIC = new BeanItemContainer<CodigosConvenioMultilateral>(CodigosConvenioMultilateral.class,
				new ArrayList<CodigosConvenioMultilateral>());
	}
										...
										try {
											if (yes) {
												if (itemsGRD.getSelectedRow() != null) {
													...
													CodigosConvenioMultilateral item = (CodigosConvenioMultilateral) itemsGRD
															.getSelectedRow();
	...
	private void modificarBTNClick() {
		try {
			...
			if (itemsGRD.getSelectedRow() != null) {
				...
				CodigosConvenioMultilateral item = (CodigosConvenioMultilateral) itemsGRD.getSelectedRow();
				item.getCodigoCm();
	...
	private void loadData() {
		try {
			...
			List<CodigosConvenioMultilateral> items = queryData();

			itemsBIC.removeAllItems();

			for (CodigosConvenioMultilateral item : items) {
	...
	private List<CodigosConvenioMultilateral> queryData() {
		try {
			...
			List<CodigosConvenioMultilateral> items = mockData(limit, offset,
		...
		return new ArrayList<CodigosConvenioMultilateral>();
	}

	// metodo que realiza el delete en la base de datos
	private void deleteItem(CodigosConvenioMultilateral item) {
		try {

			for (int i = 0; i < itemsMock.size(); i++) {
				if (itemsMock.get(i).getCodigoCm().equals(item.getCodigoCm())) {
					itemsMock.remove(i);
					return;
				}
			}
	...
	List<CodigosConvenioMultilateral> itemsMock = new ArrayList<CodigosConvenioMultilateral>();

	private List<CodigosConvenioMultilateral> mockData(int limit, int offset, CodigosConvenioMultilateralFiltro filtro) {

		if (itemsMock.size() == 0) {

			for (int i = 0; i < 500; i++) {

				CodigosConvenioMultilateral item = new CodigosConvenioMultilateral();

				item.setCodigoCm(i);
				item.setCodigoConvenio("Nombre " + i);
				item.setDescripcion("Abreviatura " + i);

				itemsMock.add(item);
			}
		}

		ArrayList<CodigosConvenioMultilateral> arrayList = new ArrayList<CodigosConvenioMultilateral>();

		for (CodigosConvenioMultilateral item : itemsMock) {

			boolean passesFilterNumero = (filtro.getCodigoCm() == null || item
					.getCodigoCm().equals(filtro.getCodigoCm()));
