package com.massoftware.windows.motivosComentarios;
...
public class WMotivosComentarios extends Window {

	private static final long serialVersionUID = -6410625501465383928L;

	// -------------------------------------------------------------

	private BeanItem<MotivosComentariosFiltro> filterBI;
	private BeanItemContainer<MotivosComentarios> itemsBIC;
	...
	public WMotivosComentarios() {
		...
		try {
			...
			nombreTXTHL = UtilUI.buildTXTHL(filterBI, "nombre", "Nombre",
					false, 15, -1, 20, false, false, null, false,
	...
	private void buildContainersItems() throws Exception {
		...
		filterBI = new BeanItem<MotivosComentariosFiltro>(new MotivosComentariosFiltro());
		itemsBIC = new BeanItemContainer<MotivosComentarios>(MotivosComentarios.class, new ArrayList<MotivosComentarios>());
	}
										...
										try {
											if (yes) {
												if (itemsGRD.getSelectedRow() != null) {
													...
													MotivosComentarios item = (MotivosComentarios) itemsGRD
															.getSelectedRow();
	...
	private void loadData() {
		try {
			...
			List<MotivosComentarios> items = queryData();

			itemsBIC.removeAllItems();

			for (MotivosComentarios item : items) {
	...
	private List<MotivosComentarios> queryData() {
		try {
			...
			List<MotivosComentarios> items = mockData(limit, offset,
		...
		return new ArrayList<MotivosComentarios>();
	}

	// metodo que realiza el delete en la base de datos
	private void deleteItem(MotivosComentarios item) {
		try {
	...
	List<MotivosComentarios> itemsMock = new ArrayList<MotivosComentarios>();

	private List<MotivosComentarios> mockData(int limit, int offset, MotivosComentariosFiltro filtro) {

		if (itemsMock.size() == 0) {

			for (int i = 0; i < 500; i++) {

				MotivosComentarios item = new MotivosComentarios();
		...
		ArrayList<MotivosComentarios> arrayList = new ArrayList<MotivosComentarios>();

		for (MotivosComentarios item : itemsMock) {
