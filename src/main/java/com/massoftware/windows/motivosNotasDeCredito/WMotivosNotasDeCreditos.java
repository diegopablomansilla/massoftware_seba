...
public class WMotivosNotasDeCreditos extends Window {

	private static final long serialVersionUID = -6410625501465383928L;

	// -------------------------------------------------------------

	private BeanItem<MotivosNotasDeCreditosFiltro> filterBI;
	private BeanItemContainer<MotivosNotasDeCreditos> itemsBIC;
	...
	public WMotivosNotasDeCreditos() {
		super();
		...
		try {
			...
			cargaTXTHL = UtilUI.buildTXTHLInteger(filterBI, "motivo",
					"Motivo", false, 5, -1, 3, false, false, null, false,
					UtilUI.EQUALS, 0, 255);
			...
			nombreTXTHL = UtilUI.buildTXTHL(filterBI, "nombre", "Nombre",
					false, 15, -1, 30, false, false, null, false,
			...
			itemsGRD.setColumns(new Object[] { "motivo", "nombre" });

			UtilUI.confColumn(itemsGRD.getColumn("motivo"), "Motivo", true,80);
			...
			order.add(new SortOrder("motivo", SortDirection.ASCENDING));
	...
	private void buildContainersItems() throws Exception {
		...
		filterBI = new BeanItem<MotivosNotasDeCreditosFiltro>(new MotivosNotasDeCreditosFiltro());
		itemsBIC = new BeanItemContainer<MotivosNotasDeCreditos>(MotivosNotasDeCreditos.class, new ArrayList<MotivosNotasDeCreditos>());
	}
										...
										try {
											if (yes) {
												if (itemsGRD.getSelectedRow() != null) {
													...
													MotivosNotasDeCreditos item = (MotivosNotasDeCreditos) itemsGRD
															.getSelectedRow();
	...
	private void loadData() {
		try {
			...
			List<MotivosNotasDeCreditos> items = queryData();

			itemsBIC.removeAllItems();

			for (MotivosNotasDeCreditos item : items) {
	...
	private List<MotivosNotasDeCreditos> queryData() {
		try {
			...
			List<MotivosNotasDeCreditos> items = mockData(limit, offset,
		...
		return new ArrayList<MotivosNotasDeCreditos>();
	}

	// metodo que realiza el delete en la base de datos
	private void deleteItem(MotivosNotasDeCreditos item) {
		try {

			for (int i = 0; i < itemsMock.size(); i++) {
				if (itemsMock.get(i).getMotivo().equals(item.getMotivo())) {
					itemsMock.remove(i);
					return;
				}
			}
	...
	List<MotivosNotasDeCreditos> itemsMock = new ArrayList<MotivosNotasDeCreditos>();

	private List<MotivosNotasDeCreditos> mockData(int limit, int offset, MotivosNotasDeCreditosFiltro filtro) {

		if (itemsMock.size() == 0) {

			for (int i = 0; i < 500; i++) {

				MotivosNotasDeCreditos item = new MotivosNotasDeCreditos();
		...
		ArrayList<MotivosNotasDeCreditos> arrayList = new ArrayList<MotivosNotasDeCreditos>();

		for (MotivosNotasDeCreditos item : itemsMock) {


			boolean passesFilterNumero = (filtro.getMotivo() == null || item
					.getMotivo().equals(filtro.getMotivo()));
