...
public class WUnidadesDeMedida extends Window {

	private static final long serialVersionUID = -6410625501465383928L;

	// -------------------------------------------------------------

	private BeanItem<UnidadesDeMedidaFiltro> filterBI;
	private BeanItemContainer<UnidadesDeMedida> itemsBIC;
	...
	public WUnidadesDeMedida() {
		...
		try {
			...
			numeroTXTHL = UtilUI.buildTXTHLInteger(filterBI, "unidadDeMedida",
			...
			itemsGRD.setColumns(new Object[] { "unidadDeMedida", "nombre", "abreviatura" });

			UtilUI.confColumn(itemsGRD.getColumn("unidadDeMedida"), "Nro.", true, 50);
			...
			order.add(new SortOrder("unidadDeMedida", SortDirection.ASCENDING));
	...
	private void buildContainersItems() throws Exception {
		...
		filterBI = new BeanItem<UnidadesDeMedidaFiltro>(new UnidadesDeMedidaFiltro());
		itemsBIC = new BeanItemContainer<UnidadesDeMedida>(UnidadesDeMedida.class,
				new ArrayList<UnidadesDeMedida>());
	}
										...
										try {
											if (yes) {
												if (itemsGRD.getSelectedRow() != null) {
													...
													UnidadesDeMedida item = (UnidadesDeMedida) itemsGRD
															.getSelectedRow();
	...
	private void modificarBTNClick() {
		try {
			...
			if (itemsGRD.getSelectedRow() != null) {
				...
				UnidadesDeMedida item = (UnidadesDeMedida) itemsGRD.getSelectedRow();
				item.getUnidadDeMedida();
	...
	private void loadData() {
		try {
			...
			List<UnidadesDeMedida> items = queryData();

			itemsBIC.removeAllItems();

			for (UnidadesDeMedida item : items) {
	...
	private List<UnidadesDeMedida> queryData() {
		try {
			...
			List<UnidadesDeMedida> items = mockData(limit, offset,
	...
	private void deleteItem(UnidadesDeMedida item) {
		try {

			for (int i = 0; i < itemsMock.size(); i++) {
				if (itemsMock.get(i).getUnidadDeMedida().equals(item.getUnidadDeMedida())) {
					itemsMock.remove(i);
					return;
				}
			}
	...
	List<UnidadesDeMedida> itemsMock = new ArrayList<UnidadesDeMedida>();

	private List<UnidadesDeMedida> mockData(int limit, int offset, UnidadesDeMedidaFiltro filtro) {
		...
		ArrayList<UnidadesDeMedida> arrayList = new ArrayList<UnidadesDeMedida>();

		for (UnidadesDeMedida item : itemsMock) {

			boolean passesFilterNumero = (filtro.getUnidadDeMedida() == null || item
					.getUnidadDeMedida().equals(filtro.getUnidadDeMedida()));
