...
public class WDepositos extends Window {

	private static final long serialVersionUID = -6410625501465383928L;

	// -------------------------------------------------------------

	private BeanItem<DepositosFiltro> filterBI;
	private BeanItemContainer<Depositos> itemsBIC;
	...
	private HorizontalLayout depositoTXTHL;
	...
	public WDepositos() {
		...
		try {
			...
			HorizontalLayout sucursalesCBXHL = UtilUI.buildCBHL(filterBI, "sucursal",
					"Sucursal", false, true, Sucursales.class, queryDataSucursales());

			ComboBox sucursalesCBX = (ComboBox) sucursalesCBXHL.getComponent(0);

			sucursalesCBX.addValueChangeListener(e -> {
				this.loadDataResetPaged();
			});

			Button sucursalBTN = (Button) sucursalesCBXHL.getComponent(1);

			sucursalBTN.addClickListener(e -> {
				this.loadDataResetPaged();
			});

			// -----------

			depositoTXTHL = UtilUI.buildTXTHLInteger(filterBI, "deposito",
					"Deposito", false, 5, -1, 3, false, false, null, false,
					UtilUI.EQUALS, 0, 255);

			TextField depositoTXT = (TextField) depositoTXTHL.getComponent(0);

			depositoTXT.addTextChangeListener(new TextChangeListener() {
				public void textChange(TextChangeEvent event) {
					try {
						depositoTXT.setValue(event.getText());
						loadDataResetPaged();
					} catch (Exception e) {
			...
			Button depositoBTN = (Button) depositoTXTHL.getComponent(1);

			depositoBTN.addClickListener(e -> {
				this.loadDataResetPaged();
			});
			...
			filaFiltroHL.addComponents(depositoTXTHL, nombreTXTHL, sucursalesCBXHL, buscarBTN);
			...
			itemsGRD.setColumns(new Object[] { "deposito","nombre","abreviatura","sucursal","modulo" });

			UtilUI.confColumn(itemsGRD.getColumn("deposito"), "Deposito", true,50);
			UtilUI.confColumn(itemsGRD.getColumn("nombre"), "Nombre.", true, 200);
			UtilUI.confColumn(itemsGRD.getColumn("abreviatura"), "Abrev.", true, 80);
			UtilUI.confColumn(itemsGRD.getColumn("sucursal"), "Sucursal",true, 100);
			UtilUI.confColumn(itemsGRD.getColumn("modulo"), "Módulo",true, 100);
	...
	private void buildContainersItems() throws Exception {
		...
		filterBI = new BeanItem<DepositosFiltro>(new DepositosFiltro());
		itemsBIC = new BeanItemContainer<Depositos>(Depositos.class,new ArrayList<Depositos>());
	}
										...
										try {
											if (yes) {
												if (itemsGRD.getSelectedRow() != null) {
													...
													Depositos item = (Depositos) itemsGRD
															.getSelectedRow();
	...
	private void modificarBTNClick() {
		try {
			...
			if (itemsGRD.getSelectedRow() != null) {
				...
				Depositos item = (Depositos) itemsGRD.getSelectedRow();
				item.getDeposito();
	...
	private void loadData() {
		try {
			...
			((Validatable) depositoTXTHL.getComponent(0)).validate();
			((Validatable) nombreTXTHL.getComponent(0)).validate();

			List<Depositos> items = queryData();

			itemsBIC.removeAllItems();

			for (Depositos item : items) {
	...
	private List<Depositos> queryData() {
		try {
			...
			List<Depositos> items = mockData(limit, offset,this.filterBI.getBean());
	...
	private void deleteItem(Depositos item) {
		try {

			for (int i = 0; i < itemsMock.size(); i++) {
				if (itemsMock.get(i).getDeposito().equals(item.getDeposito())) {
					itemsMock.remove(i);
					return;
				}
			}
	...
	List<Depositos> itemsMock = new ArrayList<Depositos>();

	private List<Depositos> mockData(int limit, int offset,
			DepositosFiltro filtro) {
		...
		ArrayList<Depositos> arrayList = new ArrayList<Depositos>();

		for (Depositos item : itemsMock) {

			boolean passesFilterNumeroPais = (filtro.getSucursal() == null || item
					.getSucursal().equals(filtro.getSucursal().getSucursal()));

			boolean passesFilterNumero = (filtro.getDeposito() == null || item
					.getDeposito().equals(filtro.getDeposito()));

			boolean passesFilterNombre = (filtro.getNombre() == null || item
					.getNombre().toLowerCase().contains(filtro.getNombre().toLowerCase()));
