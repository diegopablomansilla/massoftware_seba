...
public class WRubrosProveedores extends Window {

	private static final long serialVersionUID = -6410625501465383928L;

	// -------------------------------------------------------------

	private BeanItem<RubrosProveedoresFiltro> filterBI;
	private BeanItemContainer<RubrosProveedores> itemsBIC;
	...
	public WRubrosProveedores() {
		...
		try {
			...
			numeroTXTHL = UtilUI.buildTXTHLInteger(filterBI, "tipoDeProv",
			...
			itemsGRD.setColumns(new Object[] { "tipoDeProv", "nombre" });

			UtilUI.confColumn(itemsGRD.getColumn("tipoDeProv"), "Nro.", true, 80);
			...
			order.add(new SortOrder("tipoDeProv", SortDirection.ASCENDING));
...
//			content.setComponentAlignment(filaFiltroHL, Alignment.MIDDLE_CENTER);
	...
	private void buildContainersItems() throws Exception {
		...
		filterBI = new BeanItem<RubrosProveedoresFiltro>(new RubrosProveedoresFiltro());
		itemsBIC = new BeanItemContainer<RubrosProveedores>(RubrosProveedores.class,
				new ArrayList<RubrosProveedores>());
	}
										...
										try {
											if (yes) {
												if (itemsGRD.getSelectedRow() != null) {
													...
													RubrosProveedores item = (RubrosProveedores) itemsGRD
															.getSelectedRow();
	...
	private void modificarBTNClick() {
		try {
			...
			if (itemsGRD.getSelectedRow() != null) {
				...
				RubrosProveedores item = (RubrosProveedores) itemsGRD.getSelectedRow();
				item.getTipoDeProv();
	...
	private void loadData() {
		try {
			...
			List<RubrosProveedores> items = queryData();

			itemsBIC.removeAllItems();

			for (RubrosProveedores item : items) {
	...
	private List<RubrosProveedores> queryData() {
		try {
			...
			List<RubrosProveedores> items = mockData(limit, offset,
		...
		return new ArrayList<RubrosProveedores>();
	}

	// metodo que realiza el delete en la base de datos
	private void deleteItem(RubrosProveedores item) {
		try {

			for (int i = 0; i < itemsMock.size(); i++) {
				if (itemsMock.get(i).getTipoDeProv().equals(item.getTipoDeProv())) {
					itemsMock.remove(i);
					return;
				}
			}
	...
	List<RubrosProveedores> itemsMock = new ArrayList<RubrosProveedores>();

	private List<RubrosProveedores> mockData(int limit, int offset, RubrosProveedoresFiltro filtro) {

		if (itemsMock.size() == 0) {

			for (int i = 0; i < 500; i++) {

				RubrosProveedores item = new RubrosProveedores();

				item.setTipoDeProv(i);
		...
		ArrayList<RubrosProveedores> arrayList = new ArrayList<RubrosProveedores>();

		for (RubrosProveedores item : itemsMock) {

			boolean passesFilterNumero = (filtro.getTipoDeProv() == null || item
					.getTipoDeProv().equals(filtro.getTipoDeProv()));
