...
import com.massoftware.windows.motivosComentarios.MotivosComentarios;
...
public class WMotivoComentario extends Window {

	private static final long serialVersionUID = -6410625501465383928L;

	// -------------------------------------------------------------

	private BeanItem<MotivoComentarioFiltro> filterBI;
	private BeanItem<MotivoComentario> itemBI;
	...
	public WMotivoComentario(MotivosComentarios item) {
		super();
		
		this.mode = UPDATE_MODE;
		
		MotivoComentarioFiltro a = new MotivoComentarioFiltro();
		a.setMotivo(item.getMotivo());
		a.setNombre(item.getNombre());
		
		filterBI = new BeanItem<MotivoComentarioFiltro>(a);
		itemBI = new BeanItem<MotivoComentario>(new MotivoComentario());
	...
	public void init() {
		...
		try {
			...
			UtilUI.confWinList(this, "Motivo comentario");
...
//			});
	...
	private void loadData() {
		try {
			...
			MotivoComentario item = queryData();
			if (item != null) {
				itemBI.setBean(item);
			} else {
	...
	private MotivoComentario queryData() {
		try {
			...
			MotivoComentario item = mockData(this.filterBI.getBean());
	...
	private MotivoComentario mockData(MotivoComentarioFiltro filtro) {

		MotivoComentario item = new MotivoComentario();
